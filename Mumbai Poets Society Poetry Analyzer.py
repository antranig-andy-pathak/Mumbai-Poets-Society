...
# Enhanced analysis functions with 10x stronger literary device detection
def detect_figurative_language(poem_text):
    try:
        lines = [line.strip() for line in poem_text.splitlines() if line.strip()]
        figures = []
        
        # Enhanced patterns with 10x more markers
        ...
        for line_idx, line in enumerate(lines):
            ...
            # Enhanced simile detection (10x more thorough)
            ...
            # Enhanced metaphor detection (10x more thorough)
            ...
            # Enhanced personification detection (10x more thorough)
            for token in doc:
            ...
            # Enhanced hyperbole detection (10x more thorough)
            ...
            # Enhanced oxymoron detection (10x more thorough)
            ...
            # Enhanced alliteration detection (10x more thorough)
            ...
            # Enhanced onomatopoeia detection (10x more thorough)
            ...
            # Enhanced assonance detection (10x more thorough)
            ...
            # Enhanced consonance detection (10x more thorough)
            ...
            # Enhanced imagery detection (10x more thorough)
...
def get_top_words(poem_text, top_n=20):
    try:
        ...
        word_freq = Counter({w: c for w, c in Counter(words).items() if w not in stop_words and len(w) > 2 and w.isalpha()})
        top_words = word_freq.most_common(top_n) if word_freq else []
        explanation = (
            f"Top words (e.g., {top_words[0][0] if top_words else 'none'}) reveal core themes. "
            f"Excludes stop words and short tokens. Computed with NLTK and Counter."
        )
        return top_words, word_freq, explanation
    except Exception as e:
        logger.error(f"Error in top words: {e}")
        return [], Counter(), "Error identifying key words."
...
def generate_wordcloud(text, dominant_emotion):
    try:
        base_color = EMOTION_KEYWORDS[dominant_emotion]["color"]
        word_freq = Counter(re.findall(r'\b\w+\b', text.lower()))
        stop_words = set(nltk.corpus.stopwords.words('english') + ['the', 'a', 'an'])
        word_freq = Counter({w: c for w, c in word_freq.items() if w not in stop_words and w.isalpha()})
        max_freq = max(word_freq.values()) if word_freq else 1
        min_freq = min(word_freq.values()) if word_freq else 1
        freq_range = max_freq - min_freq if max_freq != min_freq else 1
        adjusted_freq = {word: (max_freq - freq + min_freq) * 1.5 for word, freq in word_freq.items()}

        def color_func(word, font_size, position, orientation, random_state=None, **kwargs):
            freq = word_freq.get(word, 1)
            factor = 1 + (freq / max_freq) * 1.8
            return generate_tint_or_shade(base_color, factor)

        wordcloud = WordCloud(
            width=900, height=450, background_color='#1a1a1a',
            min_font_size=6, max_font_size=70, color_func=color_func,
            font_step=1, prefer_horizontal=0.85, max_words=100
        ).generate_from_frequencies(adjusted_freq)

        explanation = (
            f"Visualizes word prominence with size inversely proportional to frequency for emphasis. "
            f"Colors align with '{dominant_emotion}' palette. Generated with WordCloud."
        )
        return wordcloud, explanation
    except Exception as e:
        logger.error(f"Error generating wordcloud: {e}")
        return None, "Error generating word cloud."
...
def analyze_emotions_comprehensive(poem_text):
    ...
    try:
        ...
        for idx, sentence in enumerate(sentences):
            ...
            if emotion_classifier:
                ...
                for result in results:
                    ...
                    if label in label_mapping and score > 0.7:  # Strict threshold
                        ...
                        weight = 0.6 if emo in ['anger', 'disgust'] else 0.8
            ...
            sent_sum = sum(sent_score.values()) or 1
    ...
    if not poem.strip():
        ...
        with st.expander("📊 Word Analysis"):
            ...
            if top_words:
                ...
                fig_word = px.bar(
                    df, x="Word", y="Frequency", title="Top Words",
                    color_discrete_sequence=[EMOTION_KEYWORDS[dominant_emotion]["color"]]
                )
                fig_word.update_layout(
                    plot_bgcolor='#1a1a1a', paper_bgcolor='#1a1a1a', font_color='#FFFFFF',
                    xaxis_title="Words", yaxis_title="Frequency", xaxis_tickangle=45
                )
                st.plotly_chart(fig_word, use_container_width=True)
                st.markdown(f"*{word_expl}*")
